...
a loadout breakdown containing one entry per model-line sharing that name (each with its own
weapon list, remaining count, and initial count), and the name of the owning component (its
`Datasheet.Name`) so downstream consumers can identify which component contributed the entry
