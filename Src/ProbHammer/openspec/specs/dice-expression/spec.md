...
Represents a 40K-notation dice or fixed-value quantity (e.g. attacks, damage characteristics) as a
domain value type: parseable from standard notation, ergonomically constructible from plain
integers, and composable via scaling and addition so that identical weapon profiles carried by
multiple models can be aggregated into a single correctly-distributed expression. TBD: expand as
this capability grows beyond its initial promotion from the simulation engine.
