...
list as a CoreRule-origin ability attached to the resulting Unit, resolving each rule's full text
from a roster-scoped index built by collecting every distinct rule entry found anywhere in the
roster. Because the roster contains only rules that actually apply to the exported army, the system
SHALL NOT apply any separate chapter, sub-faction, or game-mode gating when extracting these.

#### Scenario: An army-wide rule resolves with its full text
- **WHEN** a unit selection's `rules` list includes an entry named `"Templar Vows"` with its own
  description text
- **THEN** the resulting Unit carries a CoreRule-origin ability named `"Templar Vows"` with that
