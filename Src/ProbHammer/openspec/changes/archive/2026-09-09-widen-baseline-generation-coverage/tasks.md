...
- [x] 1.1 Widen `RuleEffectClassifier.InvulnerableSaveRangedRestricted`/`InvulnerableSaveMeleeRestricted`
...
- [x] 1.2 Add unit tests covering the plural form for both the ranged- and melee-restricted patterns,
...
- [x] 2.1 Add a `RawType` field to `CharacteristicModifierCandidate`, populated from `BsModifier.Type`
...
- [x] 2.2 Add `CharacteristicModificationResolver.ResolveVerbFromRawDelta(kind, delta) ->
      ...
      unclassified.
- [x] 2.3 Unit-test `ResolveVerbFromRawDelta` directly: a `Plain`-family case (Auric Mantle's real
...
- [x] 3.1 In `RuleEffectClassificationReport`, join each collected ability to its own Datasheet's
...
- [x] 3.2 Derive a `CharacteristicEffect` from each joined candidate via the task 2 machinery, and add
      a new report section listing structurally-derived results.
- [x] 3.3 Add the regex-vs-structural disagreement section: for a Text with both a text-classified and
...
- [x] 4.1 Run the report tool against the full live BSData clone; confirm which raw `Type` values
...
- [x] 4.2 Manually review every newly-surfaced plural-InSv Effect result, structurally-derived Effect
      result, and any regex-vs-structural disagreement.
- [x] 4.3 Check in the grown baseline (`src/ProbHammer.Web/Data/RuleEffectClassifications.json`) via
...
- [x] 5.1 Run the full test suite and confirm it's green — this change touches no runtime consumer, so
      no existing behavioral test should need updating.
- [x] 5.2 Confirm `openspec validate --strict` passes for the updated `rule-effect-classification`/
