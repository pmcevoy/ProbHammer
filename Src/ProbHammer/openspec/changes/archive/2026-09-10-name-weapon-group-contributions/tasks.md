...
- [x] 1.1 Add `Name` (string) to `WeaponContribution`
  (`src/ProbHammer.Core/Domain/Roster/AttachedUnitAggregateView.cs`).
- [x] 1.2 Add `Name` (string) to `AggregateWeaponEntry`, alongside `Profile`/`TotalAttacks`/
...
- [x] 1.3 In `AttachedUnitAggregator.BuildWeapons`, populate `WeaponContribution.Name` from the
  already-resolved `profile.Name` at the same point `PerModelAttacks` is populated.
- [x] 1.4 In `BuildWeapons`, compute each group's composite `Name` from its `Contributions` (per
...
- [x] 2.1 Update `_UnitBlock.cshtml`'s weapon-table rendering (both Ranged and Melee sections) to
...
- [x] 3.1 Search existing tests (`tests/ProbHammer.Tests/Domain/Roster/
...
- [x] 3.2 Add a unit test on `AttachedUnitAggregator.BuildWeapons` (or its existing test class)
  ...
  ("X, Y, and Z").
- [x] 3.3 Add/update a `_UnitBlock.cshtml` rendering test confirming the composite Name renders in
  the weapon name cell and as the breakdown-toggle trigger text.
- [x] 3.4 Run the full test suite and confirm no regressions.

## 4. Verification

- [x] 4.1 `dotnet run` + browser check against a real captured export — confirm ordinary
  ...
  Verified against `data/gw-app-export-templars-latest.txt` via `dotnet run` +
  `firefox-devtools` MCP: every weapon-name cell across all five unit blocks rendered its plain
  single Name unchanged (no stray "and"/Oxford-comma joins), including several merges of the
  identically-named "Master-crafted Power Weapon" across different components. No
  differently-named structural merge exists in this particular captured export (expected - see
  design.md's own Risk note that no corpus-frequency audit was required).

## 5. Docs

- [x] 5.1 Update `.claude/domain-model/roster-context.md` (or wherever `AggregateWeaponEntry`/
  `WeaponContribution` are documented) to describe the new `Name` fields and composite-naming
  behavior.
- [x] 5.2 Trim the completed Phase 0 bullet out of `.claude/vnext-ideas.md`'s "WeaponProfile-
