...
- [x] 1.1 Confirm `compress-loadout-labels` is implemented and archived; reconcile this change's
...
- [x] 2.1 Add a `LoadoutIndex` (or equivalent) field to `WeaponContribution`, set in
...
- [x] 3.1 Thread `(ComponentName, StatlineName, LoadoutIndex)` identity onto rendered statline
      header/loadout elements and contribution-breakdown rows in `LivePlay.cshtml` as `data-*`
      attributes.
- [x] 3.2 Ensure the server still renders ungrouped, per-`LoadoutIndex` contribution rows (hidden
...
- [x] 4.1 Make each statline-header line and loadout `<li>` a single click/tap target.
- [x] 4.2 Render tri-state visual indicator (selected / deselected / partial) per statline; simple
      checked/unchecked indicator per loadout.
- [x] 4.3 Implement click handling in `live-play.js`: loadout toggles its own selection only;
      statline header toggles all its loadouts per the fully-selected/otherwise rule from design.md.
- [x] 4.4 Render the Statline section's live "N/M selected" badge, updated on every toggle.

## 5. Weapon sections filtering

- [x] 5.1 Implement per-row recompute in `live-play.js`: sum currently-selected contribution
      subtotals per weapon row; hide the row when the selected sum has zero contributing entries.
- [x] 5.2 Implement breakdown-group re-merge/split on the client: collapse when visible
...
- [x] 5.3 Render Ranged/Melee section "filtered" badges, computed per-section from whether any row
      in that section changed under the current selection.

## 6. Clear filter control

- [x] 6.1 Render a per-unit-block "Clear filter" control, visible only while that block's selection
      isn't fully selected; wire it to reset the block's deselected-keys state and re-run the
      recompute/badge pass.

## 7. Tests

- [x] 7.1 Unit test: `WeaponContribution.LoadoutIndex` is set correctly for the real Crusader Squad
      fixture (two Initiate loadouts get distinct indices).
- [x] 7.2 Unit test: contribution-breakdown grouping/splitting logic (wherever it's tested
      ...
      coverage.
- [x] 7.3 Manual/scripted check of `live-play.js`'s recompute logic against the worked Crusader
...
- [x] 8.1 Run the full test suite; confirm no regressions. (258/258 passing)
- [x] 8.2 Visually verify via `firefox-devtools-mcp` against the real example army: default
