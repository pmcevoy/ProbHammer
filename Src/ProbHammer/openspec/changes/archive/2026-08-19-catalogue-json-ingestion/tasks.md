...
- [x] 1.1 Implement the "get JSON content for this filename" boundary (design.md) as a small
...
- [x] 1.2 Define minimal deserialization types for the BattleScribe `catalogueSchema` JSON shape
...
- [x] 1.3 Deliberately omit deserialization of `constraints`, `modifiers`, `conditionGroups`, and
...
- [x] 1.4 Load and deserialize a real sample file end-to-end (`Imperium - Imperial Fists.json`,
      the smallest) as a smoke test before tackling the larger files.
- [x] 1.5 Copy the specific real-data excerpts this change's automated tests depend on (e.g. the
...
- [x] 2.1 Implement closure resolution: given a starting file, follow every `catalogueLinks` entry
      with `importRootEntries: true`, recursively, collecting the full set of files.
- [x] 2.2 Verify against real data: closure starting from `Imperium - Black Templars.json`
...
- [x] 2.3 Verify against real data: closure starting from `Chaos - Chaos Space Marines.json`
...
- [x] 2.4 Guard against import cycles (not observed in the sampled data, but not ruled out either).

## 3. Name Resolution

- [x] 3.1 Implement name resolution over a closure: check the starting file's own
      `sharedSelectionEntries` first; only on a miss, check each imported file in declared
      `catalogueLinks` order.
- [x] 3.2 Verify against real data: resolving `"Crusader Squad"` against the Black Templars
      closure returns the locally-defined entry, not anything from the Space Marines import.
- [x] 3.3 Verify against real data: resolving `"Assault Intercessor Squad"` and `"Scout Squad"`
      against the Black Templars closure falls through to the Space Marines import (neither name
      appears locally at all).
- [x] 3.4 Trace whether `entryLink`s always carry a `targetId`, or whether some name lookups need
...
- [x] 4.1 Implement single-model detection and extraction: a `Unit`-typeName profile on the
      resolved entry itself becomes one named `Statline`.
- [x] 4.2 Implement squad detection and extraction: `Unit`-typeName profiles nested on distinct
      child entries become one named `Statline` each, preserving declared order.
- [x] 4.3 Map `M`, `T`, `Sv`, `W`, `LD`, `OC` characteristics onto `Statline`'s fields; map `InSv`
      directly from its own characteristic (no text extraction needed, unlike 10e).
- [x] 4.4 Trace the four `Initiate` occurrences inside `Crusader Squad`
...
- [x] 5.1 Implement `Ranged Weapons`-typeName profile mapping to `RangedWeapon`
      (`Range`, `A`, `BS`, `S`, `AP`, `D`).
- [x] 5.2 Implement `Melee Weapons`-typeName profile mapping to `MeleeWeapon`
      (`A`, `WS`, `S`, `AP`, `D`; `Range` characteristic literal `"Melee"` is not stored, matching
      `MeleeWeapon`'s existing fixed `Range = 0`).
- [x] 5.3 Confirm AP values pass through unchanged as negative integers (source data already uses
      the same sign convention as `WeaponProfile.Ap`).

## 6. Weapon Keyword Text and Flag Parsing

- [x] 6.1 Add a verbatim, always-populated keyword-text field to `WeaponProfile`
      (`datasheet-catalogue` delta spec's "Weapon Profile Verbatim Keyword Text" requirement) and
      include it in `EqualityKey()`.
- [x] 6.2 Implement the `Keywords` free-text split (comma-separated tokens) and populate the new
      verbatim field with every token, in source order, unconditionally.
- [x] 6.3 Implement per-token pattern match against `WeaponProfile`'s existing flags (`Torrent`,
...
- [x] 6.4 Handle `"-"` as "no keywords" (produces no flags set and an empty verbatim list, not a
      parse failure).
- [x] 6.5 Verify against real data: `"Anti-infantry 4+, Devastating Wounds"`,
...
- [x] 7.1 Implement `Abilities`-typeName profile mapping to `Ability`, carrying the `Description`
      characteristic through as `Text` verbatim.
- [x] 7.2 Confirm no attempt is made to parse `Text` for behavioral effect, per existing
      `datasheet-catalogue` stance.

## 8. Cross-Cutting Verification

- [x] 8.1 Confirm no `constraints`, `modifiers`, `conditionGroups`, or `associations` data leaks
      onto any produced `Datasheet`, `Statline`, `WeaponProfile`, or `Ability`.
- [x] 8.2 Confirm no points/`costs` data is exposed on any produced object.
- [x] 8.3 End-to-end test: resolve every named unit/model/weapon that appears in
