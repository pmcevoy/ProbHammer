...
/// <summary>
/// Curated, per-faction table of known army-wide rule names - the sole source of truth for
/// classifying a Core Rule Ability Extraction reference's Origin as ArmyRule (see
/// <c>BsdataDatasheetMapper</c>'s "Core Rule Ability Extraction" and
...
/// composition rules, not gameplay rules) - consulted only by an offline corpus-scan triage check,
/// never at runtime: a name simply absent from <see cref="InclusionTable"/> already classifies
/// Core Rule by default.
/// </summary>
public static class ArmyRuleNameLookup
{
    ...
    /// is therefore its own most-specific Faction entry.</summary>
    private static readonly IReadOnlyDictionary<string, IReadOnlyList<string>> InclusionTable =
        new Dictionary<string, IReadOnlyList<string>>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            // its Name (not a modifier-appended display name) - confirmed in both the BSData
            // catalogue and a real captured roster export.
            ["Death Guard"] = ["Nurgle's Gift (Aura)"],
            // Real BSData catalogue file and export catalogueName leaf are "Adeptus Custodes",
            // not the shorter "Custodes".
            ...
            // Both rule definitions live in the shared "Library - Tyranids.json" file, gated by
            ...
            // origin abilities in one faction.
            ["Tyranids"] = ["Synapse", "Shadow in the Warp"],
            // Every row from here down was independently validated against Wahapedia (not just
            // the frequency + "If your Army Faction is..." text-shape signal that surfaced it as
            // a candidate), ordered alphabetically by faction.
            ...
            // Confirmed via Wahapedia validation to be a genuine ArmyRule, not a mustering/
            // composition rule despite sharing that gating shape. Its own rule text opens with
            // the identical "If your Army Faction is Astra Militarum..." signature every other
            ...
            // Confirmed via Wahapedia validation: no single unifying army-wide rule exists for
            // this catalogue (a rules-light bucket for unaligned/renegade allies, not a real
            // playable army in the same sense as the rows above) - an explicit empty entry, not
            // an absent key, distinguishing "checked, found nothing" from "nobody has looked yet".
            ["Unaligned Forces"] = [],
            // The candidate rule name surfaced by external research ("Towering Example") does
            // not appear anywhere in the bundled BSData corpus snapshot. Adding an unverifiable
            // name would violate this table's own "verified to resolve" requirement, so this is
            // an explicit empty entry rather than a guess.
            ["Adeptus Titanicus"] = [],
            ["Titanicus Traitoris"] = [],
    ...
    /// Consulted only by an offline corpus-scan triage check, never by
    ...
    /// across every faction - used only by an offline corpus-scan triage check to filter out
    /// gated rules already accounted for by the inclusion table before checking what remains
    /// against the exclusion list.</summary>
    ...
    /// <summary>Every (Faction, Names) entry in <see cref="InclusionTable"/>, exposed only so an
    /// offline corpus scan can resolve each entry's own faction against the live BSData clone and
    /// re-verify its names still resolve - ordinary runtime callers use <see cref="Resolve"/>
    /// instead.</summary>
