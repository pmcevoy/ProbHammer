...
/// <summary>One human-verified rule/ability classification record in the checked-in baseline - the
/// JSON counterpart of a <see cref="RuleClassification"/>, keyed by the rule/ability's own normalized
/// Text, never Name (<see cref="RuleEffectClassifier.Classify"/> never reads its <c>name</c> argument,
/// so two different Names sharing identical Text are the same classification result).
///
/// <see cref="Names"/> is purely a human-findability aid, NOT part of this entry's identity - Text
/// remains the sole key, and two entries can never differ only by Names. Without it, locating a
/// specific entry in the checked-in JSON meant searching for a snippet of its (sometimes long,
/// sometimes near-duplicate) Text; a real ability/rule Name is what a person actually remembers and
/// searches for. Refreshed by `--write-baseline` exactly like `Target`/`Effects`/`IsCaveated` (it's a
/// corpus-derived fact, not a human judgment call, unlike `FullyHandled`/`Note` below) - every distinct
/// Name currently seen carrying this Text, sorted, so a text shared by several differently-named
/// abilities lists all of them, not just one. Defaults to empty for an entry added by hand before its
/// first `--write-baseline` refresh. <see cref="Note"/> is optional free text recording that this
/// entry's classification, while verified correct as far as it goes, is known to omit real content its
/// text states; not present on most entries.
///
/// <see cref="FullyHandled"/> answers a different question than <see cref="Classification"/>'s own
/// <see cref="RuleClassification.IsCaveated"/>: IsCaveated is a purely mechanical, text-only fact ("is
/// there text left over after the last match"), computed fresh every run with no baseline awareness -
/// it can never know whether that leftover text is real, unextracted game content (a keyword grant, a
/// recurring effect the player genuinely needs to read) or something the classifier's vocabulary was
/// never meant to cover at all (an army-composition eligibility restriction, flavor text). FullyHandled
/// is that second, human-only verdict: true means a person has confirmed the leftover text names no
/// further game effect, so a downstream consumer should treat this as fully resolved and never prompt
/// the player to re-read the ability, even though IsCaveated stays true forever.
///
/// Not a field on <see cref="RuleClassification"/> itself, since it has no freshly-computed value and
/// so never enters <see cref="RuleClassificationDiff"/>'s comparison. Defaults false; like
/// <see cref="Note"/>, never touched by `--write-baseline`'s refresh loop, since neither field is
/// something a classification run could derive on its own.</summary>
...
/// <summary>A checked-in, Text-keyed record of human-verified <see cref="RuleClassification"/>s,
/// so a reviewer of the corpus report never has to re-read an already-verified, unchanged result.
...
public sealed class RuleClassificationBaseline
{
    ...
    /// property names and indented output, so a snapshot write produces a readable <c>git diff</c>.
    /// The relaxed encoder keeps ordinary punctuation real BSData
