# Visual Design Specification — Archived Dark Theme

> Archived alongside the `/Index`/`/ArmyView` pages this theme was built for, by the
> `archive-10e-pipeline` change (and this follow-up). `/LivePlay`'s own visual design — the only
> live theme today — is documented at `.claude/design-tokens.md`. Checked directly against
> `site.css` before moving this: `.live-play-page`'s CSS block is fully self-contained (its own
> border-radius/spacing/hover-state values, declared independently) and the file's one `@media`
> breakpoint only targets the archived `.army-columns`/`.army-inputs`/`.catalogue-bar` selectors —
> nothing below actually describes live `/LivePlay` behavior, direct or inherited, despite the
> original file's "Live Play Page Exception" section being written as a contrast against it.
