...
WeaponKeywordParser.Apply(weapon, keywordsText) -> WeaponProfile
                                       // see class's and method's own doc comment. Pure
                                       // tokenization: splits Keywords on ","; "-"/blank means
                                       // none. No per-token recognition or flag mapping -
                                       // KeywordsText is the sole representation of a weapon's
                                       // ability keywords.
...
`Bsdata/`) is the sole representation of a weapon's ability keywords — exact source keyword text,
in source order. `WeaponProfile.EqualityKey()` groups by a case-folded, deduplicated,
sorted-then-joined normalization of `KeywordsText` (`WeaponProfile.NormaliseKeywords`, reusing
`RuleGlossary.NormalizeToken`), so two profiles spelled or cased differently but carrying the same
real keywords still aggregate into one entry, while any real keyword difference still splits
them. `_UnitBlock.cshtml`'s two weapon-tag render sites read `weapon.KeywordsText` directly: every
token always renders as a chip, resolved (clickable, popover) or unresolved (dimmed) per
`RuleGlossary.TryResolve`, never silently dropped regardless of whether it corresponds to a
modeled mechanic.
...
WeaponKeywordScanTests.Full_corpus_weapon_keyword_token_scan
                                       ...
                                       // ResolveWeaponProfile is on-demand only). Tokenizes every
                                       // weapon's Keywords text and checks each token against
                                       // that same closure's own RuleGlossary.Build(closure),
                                       // recording one with no TryResolve match.
                                       // WeaponKeywordAllowlist.cs (see its own class doc
                                       // comment) is a maintained list of tokens already
                                       // confirmed to have no matching BSData rule/sharedRule -
                                       // purely a test-tracking artifact for this manual scan, so
                                       // a genuinely new gap is distinguishable from an
                                       // already-known one; it has no effect on production
                                       // rendering, where an unresolved token always renders as a
                                       // dimmed, non-interactive chip either way, allowlisted or
                                       // not.
```
