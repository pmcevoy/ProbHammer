...
/// <summary>
/// Permanent, manually-triggered regression scan over the full local BSData clone - the
/// "Detachment" selection-group name match is confirmed real in Space Marines but is not otherwise
/// exhaustively checked against every faction file in the live clone. Same pattern as
/// CharacteristicResolutionScanTests/WeaponKeywordScanTests: every real faction file (excluding
/// "Library" files - shared cross-sub-faction content, never a playable faction identity itself,
/// matching BsdataFactionResolver's own exclusion) gets a turn as its own closure's starting file,
/// and its closure must resolve at least one Detachment entry via
