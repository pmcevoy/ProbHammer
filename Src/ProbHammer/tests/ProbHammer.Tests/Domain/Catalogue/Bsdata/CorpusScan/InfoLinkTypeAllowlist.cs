...
/// shape and must be triaged deliberately, not silently dropped.
/// </summary>
public static class InfoLinkTypeAllowlist
{
    ...
    [
        ...
        // Confirmed real gap: a 129-occurrence pattern in the live clone, deliberately NOT fixed
        // here. Targets a `sharedInfoGroups`/`infoGroups` container holding its own nested
        // `profiles` array of real Abilities-typeName content (confirmed real example: Adeptus
        // Custodes' "Talons" info group holds two genuine, real-text, mutually-exclusive-by-
        // modifier auras - "Null Aegis (Aura)" and "Deadly Unity (Aura)" - currently invisible on
        // /LivePlay). Verifying a fix needs a real Custodes (or other infoGroup-carrying faction)
        // export to check against, which wasn't available - tracked as a dedicated follow-up
        // instead of fixed blind.
        new(
