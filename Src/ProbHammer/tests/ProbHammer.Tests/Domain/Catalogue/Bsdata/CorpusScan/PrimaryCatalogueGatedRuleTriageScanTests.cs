...
/// <summary>
/// Permanent, manually-triggered regression scan over the full local BSData clone. Collects every
/// rule definition anywhere in the clone (every real catalogue file's own local <c>rules</c>/
/// <c>sharedRules</c>, plus the game system's own <c>sharedRules</c>, deduped by id/name across
/// the whole run) carrying the same "primary-catalogue"-scoped gating shape and requires each to
/// be accounted for by either <see cref="ArmyRuleNameLookup"/>'s curated inclusion table or its
/// curated exclusion list - failing on anything neither covers, so a future BSData/codex update's
/// new shared-library rule gets flagged for a human to triage rather than silently doing nothing
/// or silently misclassifying, the way a structural-only signal alone would for Assigned
/// Agents/Disparate Paths/Corsairs and Travelling Players.
///
...
public class PrimaryCatalogueGatedRuleTriageScanTests
{
    ...
    /// <summary>The same structural signal Origin classification once relied on in production,
    /// before it moved to name-based classification - duplicated here rather than reused from
    /// production code, matching this project's existing convention of a corpus scan owning its
    /// own self-contained walk (see <c>WeaponKeywordScanTests</c>).</summary>
